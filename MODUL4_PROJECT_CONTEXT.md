...
Админ редактирует локально через визуальный HTML-редактор `board-editor.html` (File System Access API, Chrome/Edge) и пушит изменения на GitHub
...
Рабочий процесс админа (основной — через локальный редактор):

1. Открыть `board-editor.html` в Chrome или Edge (файл хранится локально у админа, исключён из Git через `.gitignore`)
2. Нажать «Открыть папку проекта» → выбрать корневую папку репозитория SatContact
3. Редактор загружает `data/board.html` в визуальный WYSIWYG-редактор (contentEditable)
4. Форматирование текста: жирный, курсив, ссылки, разделители (панель инструментов)
5. Добавление медиа: кнопки 🖼 📹 📎 → выбор файлов → файлы автоматически копируются в `data/board-media/` с транслитерированными именами (латиница), в документ вставляются корректные HTML-теги (`<img>`, `<video>`, `<a>`)
6. Поддержка drag & drop медиафайлов прямо в область редактирования
7. Панель превью, просмотр и редактирование HTML-исходника, список файлов в `board-media/`
8. Нажать «Сохранить» → изменения записываются в `data/board.html`
9. Выполнить `git add`, `git commit`, `git push` (или через GitHub Desktop)
10. Обновление на сайте через ~1-2 мин (GitHub Pages deploy)

Важно: имена медиафайлов должны быть в латинице. Редактор автоматически транслитерирует имена (заменяет нелатинские символы). GitHub Pages использует Jekyll, который игнорирует файлы с именами, начинающимися с `_` (нижнее подчёркивание) — такие файлы не попадут на сайт.

Резервный способ (через GitHub.com, без редактора):

1. Открыть github.com → репозиторий → `data/board.html` → карандаш (Edit)
2. Отредактировать HTML-код вручную
3. Commit changes
4. Для загрузки медиа: `data/board-media/` → Add file → Upload files → Commit (имена файлов — латиница)
5. Обновление на сайте через ~1-2 мин (GitHub Pages deploy)
...
**Детекция обновлений доски (`checkBoardForUpdates`) — перенести в SW:**
Текущая реализация (news.js) имеет ограничения, которые решаются на уровне Service Worker:
1. Проверка выполняется однократно при старте — SW обеспечит периодическую фоновую проверку.
2. Баг пустого кэша: условие `if (cached && cached !== html)` не срабатывает при первом визите (пустая строка falsy) — SW должен сохранять baseline при первом fetch, а не игнорировать отсутствие кэша.
3. CDN GitHub Pages может отдавать устаревшую версию даже с `cache: 'no-cache'` — SW должен использовать cache-busting (query-параметр) или ETag-проверку.
SW уведомляет основной поток через `postMessage` → клиент переключает класс `btn--news-updated`.
...
data/board.html	HTML-фрагмент доски объявлений (редактирует админ через board-editor.html или GitHub)
data/board-media/	Папка для медиафайлов доски (файлы копируются редактором или загружаются через GitHub; имена — латиница)
board-editor.html	Локальный WYSIWYG-редактор доски (File System Access API, Chrome/Edge). Исключён из Git (.gitignore), хранится только на ПК админа
...
✅ Создан. Редактируется админом через локальный визуальный редактор `board-editor.html` (основной способ) или через web-интерфейс GitHub.com (резервный способ).
2. Папка data/board-media/
✅ Создана. Содержит `.gitkeep` и медиафайлы доски. Файлы копируются автоматически редактором `board-editor.html` при добавлении медиа, или загружаются вручную через GitHub.com. Имена файлов — строго латиница (Jekyll/GitHub Pages игнорирует файлы, начинающиеся с `_`).
3. board-editor.html (Локальный редактор доски)
✅ Создан. Однофайловый WYSIWYG HTML-редактор, работающий через File System Access API (Chrome/Edge). Исключён из Git через `.gitignore` — хранится только на ПК админа. Возможности: визуальное редактирование, вставка медиа (изображения, видео, файлы) с автокопированием в `data/board-media/`, drag & drop, просмотр HTML-исходника, превью, список файлов в `board-media/`.
