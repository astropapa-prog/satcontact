...
`data/board.html`, `data/Frequencies.xml` и `data/board-media/*` обновляются исключительно через деплой (push в GitHub Pages) + бамп `CACHE_VERSION` в `sw.js`. Runtime-детекция обновлений, поллинг и уведомления (`handleBoardFetch`, `BOARD_UPDATED`, `triggerBoardCheck`, `.btn--news-updated`) полностью удалены.
...
|------|-----|-----|------|
| Precache | `satcontact-v7` (версионируется) | Да — старая версия удаляется в `activate` | App shell: HTML, CSS, JS, XML, JSON, board.html, board-media, иконки, манифест (24 файла) |
| Runtime | `satcontact-runtime` (постоянный) | Нет — живёт между версиями SW | `data/tle.txt` |

| Ресурс | Стратегия SW | Кэш |
|--------|-------------|------|
| `index.html`, `*.js`, `*.css`, `manifest.json`, иконки | Cache First (precache) | `satcontact-v7` |
| `data/countries-50m.json`, `data/Frequencies.xml`, `data/board.html`, `data/board-media/*` | Cache First (precache) | `satcontact-v7` |
...
**Двойной кэш:** `satcontact-v7` (versioned precache, 24 файла app shell включая `board.html`) + `satcontact-runtime` (persistent, только `data/tle.txt`). При обновлении SW старый precache удаляется в `activate`, runtime переживает обновления.
...
2. Слушатель `controllerchange` — если SW обновился (`skipWaiting` + `clients.claim`), флаг сбрасывается в `null`. При `visibilitychange → visible` с `null`-флагом → `location.reload()`.
