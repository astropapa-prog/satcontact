...
├── sw.js                # Service Worker: precache + runtime cache, стратегии fetch, детекция обновлений доски
├── icons/
...
### 7.6 Детекция обновлений доски (через Service Worker)

Детекция обновлений доски объявлений реализована в Service Worker (`sw.js`, функция `handleBoardFetch`). SW перехватывает каждый fetch `data/board.html`, сравнивает ответ сервера с кэшированной версией в `satcontact-runtime` и уведомляет клиент через `postMessage({ type: 'BOARD_UPDATED' })`.

**Поток:**
1. `init()` в `app.js` выполняет `fetch('data/board.html')` — триггер проверки.
2. SW перехватывает → `handleBoardFetch` → `fetch(request, { cache: 'no-store' })` — обходит HTTP-кэш и CDN GitHub Pages.
3. Если кэша нет (первый визит) — сохраняет baseline в `satcontact-runtime`, без уведомления.
4. Если HTML изменился — `postMessage` всем клиентам → слушатель в `app.js` добавляет `.btn--news-updated` на `#newsBtn`.
5. Если HTML совпадает — обновляет кэш, без уведомления.
6. Если офлайн — отдаёт кэшированную версию.

**Триггеры проверки:** при старте приложения (`init()`), при открытии доски (`loadBoard()`), после авто-перезагрузки по `visibilitychange`.

Шиммер снимается в `loadBoard()` (news.js) при просмотре доски: `btn.classList.remove('btn--news-updated')`.

Функция `checkBoardForUpdates()` удалена из `news.js` — вся логика сравнения перенесена в SW.
...
|------|-----|-----|------|
| Precache | `satcontact-v2` (версионируется) | Да — старая версия удаляется в `activate` | App shell: HTML, CSS, JS, XML, JSON, иконки, манифест (23 файла) |
| Runtime | `satcontact-runtime` (постоянный) | Нет — живёт между версиями SW | `data/tle.txt`, `data/board.html`, `data/board-media/*`, `data/Frequencies.xml` |

| Ресурс | Стратегия SW | Кэш |
|--------|-------------|------|
| `index.html`, `*.js`, `*.css`, `manifest.json`, иконки | Cache First (precache) | `satcontact-v2` |
| `data/countries-50m.json` | Cache First (precache) | `satcontact-v2` |
| `data/Frequencies.xml` | Network First (precached для офлайн-baseline) | `satcontact-runtime` |
| `data/tle.txt` | Network First | `satcontact-runtime` |
| `data/board.html` | Network First + детекция обновлений (`handleBoardFetch`) | `satcontact-runtime` |
| `data/board-media/*` | Network First | `satcontact-runtime` |
...
`window.checkBoardForUpdates()` удалён — детекция обновлений доски перенесена в Service Worker (см. 7.6).
...
Приложение работает как Progressive Web App: офлайн-кеширование, установка на домашний экран, автоматическое обновление при деплое. Обновления редкие (~раз в 2 месяца), версия `CACHE_VERSION` в `sw.js` меняется вручную.

### 8.2 Service Worker (sw.js)

**Двойной кэш:** `satcontact-v2` (versioned precache, 23 файла app shell) + `satcontact-runtime` (persistent, данные Network First). При обновлении SW старый precache удаляется в `activate`, runtime переживает обновления.
...
- `data/board.html` → `handleBoardFetch()` — Network First + cache-busting (`?_=timestamp`) + детекция обновлений + `postMessage` (см. 7.6).
- `data/tle.txt`, `data/board-media/*`, `data/Frequencies.xml` → `networkFirst()` → `satcontact-runtime`.
- Всё остальное → `cacheFirst()` → поиск во всех кэшах.

**networkFirst:** `fetch(request, { cache: 'no-store' })` — полностью обходит HTTP-кеш браузера и CDN GitHub Pages (max-age ~10 мин). Без этого флага `cache: 'default'` мог вернуть ответ из HTTP-кеша, не обращаясь к сети.
...
1. При загрузке запоминается `navigator.serviceWorker.controller`.
2. Слушатель `controllerchange` — если SW обновился (`skipWaiting` + `clients.claim`), флаг сбрасывается в `null`.
3. Слушатель `visibilitychange` — при возврате в приложение (`visible`):
   - Если флаг `null` (SW обновился) → `location.reload()`. Страница загружается из нового кэша.
   - Иначе: `fetch('data/board.html')` с троттлингом 5 минут (`BOARD_CHECK_INTERVAL`) — триггер для SW-сравнения с кэшированной версией доски.
4. Слушатель `message` — приём `{ type: 'BOARD_UPDATED' }` от SW → добавление `.btn--news-updated` на `#newsBtn`.
...
1. Разработчик пушит в main (новый `CACHE_VERSION` в `sw.js`, обновлённые файлы).
2. GitHub Pages публикует.
3. Браузер проверяет `sw.js` → файл изменился → новый SW: install (precache с `cache: 'reload'`) → `skipWaiting` → activate (удаление старого кэша) → `clients.claim`.
4. Событие `controllerchange` в клиенте → флаг сбрасывается.
5. При возврате в приложение (`visibilitychange → visible`) → `location.reload()` → страница из нового кэша.

### 8.8 Precache-список (23 файла)

`./` (index.html), `style.css`, `utils.js`, `app.js`, `tle.js`, `tle-worker.js`, `gps-service.js`, `map.js`, `map-render.js`, `ar.js`, `ar-render.js`, `news.js`, `lib/satellite.min.js`, `lib/d3.min.js`, `lib/topojson.min.js`, `lib/aws4fetch.min.js`, `data/Frequencies.xml`, `data/countries-50m.json`, `data/board-media/ava.jpg`, `manifest.json`, `icons/icon-192.png`, `icons/icon-512.png`, `icons/icon-180.png`.
...
- В `app.js`: `#headerTitle` заменён на `#newsBtn`, убран keydown-обработчик (кнопка нативно реагирует на Enter/Space). Триггер проверки доски при загрузке (исходно `checkBoardForUpdates()`, заменён на `fetch('data/board.html')` в PWA-миграции).
...
- Вызывается из `app.js` при инициализации приложения **однократно** — нет периодической проверки.
- **Исправлено в PWA-миграции:** функция `checkBoardForUpdates()` удалена, детекция перенесена в Service Worker (см. раздел 8, сессия PWA-миграции).
...
- `sw.js` — Service Worker с двойным кэшем: `satcontact-v1` (versioned precache, 23 файла) + `satcontact-runtime` (persistent, Network First для `tle.txt`, `board.html`, `board-media/*`). Стратегии: Cache First для app shell, Network First для данных, `handleBoardFetch` для доски (сравнение HTML + `postMessage`), пропуск cross-origin. `skipWaiting()` + `clients.claim()` для немедленной активации.
...
**4. app.js**
- В начало IIFE (после `'use strict'`): блок регистрации SW — `controllerchange` (сброс флага), `visibilitychange` (авто-перезагрузка при возврате), `message` (приём `BOARD_UPDATED` от SW → шиммер на `#newsBtn`), `register('sw.js')`.
...
- `loadBoard()` без изменений — снимает `.btn--news-updated` как прежде.
...
- `handleBoardFetch`: cache-busting `?_=timestamp` к URL запроса + стабильный `cacheKey` (URL без query) для `cache.match`/`cache.put`.
- `visibilitychange` в app.js: при возврате пользователя в PWA из фона — `fetch('data/board.html')` с троттлингом 5 минут (`BOARD_CHECK_INTERVAL`) — триггер для SW-сравнения с кэшированной версией доски.
