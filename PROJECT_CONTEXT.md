...
**Известные ограничения текущей реализации (исправить при миграции на PWA):**
- Проверка выполняется **однократно** при старте приложения (`app.js init()`). Нет периодической проверки — если `board.html` обновится после загрузки страницы, кнопка не отреагирует до перезагрузки.
- **Баг пустого кэша:** условие `if (cached && cached !== html)` не срабатывает при первом визите (когда в localStorage ещё нет `satcontact_board_html`), потому что пустая строка — falsy. При миграции на SW нужно сохранять baseline при первом fetch, а не игнорировать отсутствие кэша.
- CDN GitHub Pages может отдавать закэшированную версию `board.html` даже с `{ cache: 'no-cache' }` — нужен cache-busting (query-параметр с таймстампом) или ETag-проверка на уровне SW.

**TODO [PWA]:** при реализации Service Worker перенести детекцию в SW (Background Sync или периодический fetch → postMessage в клиент). Все три ограничения выше решаются архитектурно на уровне SW.
...
- **TODO [PWA]:** перенести `checkBoardForUpdates()` из клиентского fetch в Service Worker для фоновой детекции обновлений доски объявлений. При реализации исправить: (1) однократную проверку заменить на периодическую, (2) баг пустого кэша — `if (cached && cached !== html)` не срабатывает при первом визите (пустая строка falsy), нужно сохранять baseline при первом fetch, (3) добавить cache-busting для обхода CDN GitHub Pages.
...
- **Известный баг:** условие `if (cached && cached !== html)` не срабатывает при первом визите (пустой кэш = falsy). Не исправляется до миграции на PWA.
- **TODO [PWA]:** перенести в Service Worker для фоновой детекции. При реализации: добавить периодичность, исправить баг пустого кэша (сохранять baseline при первом fetch), добавить cache-busting для CDN.
