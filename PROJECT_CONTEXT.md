...
- **news.js** — вся логика модуля: доска, чат, авторизация по позывному + паролю (хеш из `callsign:password`), визуальная идентификация (hashToColor, blockies-аватарки), CRUD сообщений, ответы с цитированием, личные сообщения (DM), фильтры («Все»/«Ответы мне»/«Личные»), поллинг, сжатие изображений, ленивая загрузка секций, детекция обновлений доски.
...
- Повторный разворот чата: `checkForNewMessages()` + `startPolling()`.
...
| Что | Где | TTL |
|-----|-----|-----|
| Листинг файлов (ListObjectsV2): `{ ts, files, dmFiles }` | sessionStorage | 90 сек |
...
Каждые 90 секунд — фоновый ListObjectsV2. **Только при развёрнутом чате.** Листинг захватывает и `-msg.json`, и `-dm.json` файлы. Новые публичные сообщения добавляются в `loadedMessages`, новые DM (если `allDmsLoaded`) — проверяются на принадлежность и добавляются в `loadedDmMessages`. Удалённые файлы фильтруются из обоих массивов. При скрытой вкладке поллинг приостанавливается (Visibility API). При сворачивании чата — поллинг останавливается. При развороте — возобновляется с немедленным запросом.
...
**Переменные состояния (news.js):** `pendingReply`, `activeFilter`, `allDmFiles`, `loadedDmMessages`, `allMessagesLoaded`, `allDmsLoaded`.

**Ключевые функции (news.js):** `getFilteredMessages()`, `getEmptyMessage()`, `ensureAllMessagesLoaded()`, `ensureAllDmsLoaded()`, `setFilter()`, `setPendingReply()`, `sortByTimestamp()`.
