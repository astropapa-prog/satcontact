...
- **news.js** — вся логика модуля: доска, чат, авторизация по позывному + паролю (хеш из `callsign:password`), визуальная идентификация (hashToColor, blockies-аватарки), CRUD сообщений, поллинг, сжатие изображений, ленивая загрузка секций, детекция обновлений доски.
