...
- **news.js** — вся логика модуля: доска, чат, авторизация по позывному, CRUD сообщений, поллинг, сжатие изображений, ленивая загрузка секций, детекция обновлений доски.
...
- Панель ввода (`#newsInputBar`) скрыта при свёрнутом чате.
...
Каждые 90 секунд — фоновый ListObjectsV2. **Только при развёрнутом чате.** Новые сообщения добавляются в ленту, удалённые — убираются из DOM. При скрытой вкладке поллинг приостанавливается (Visibility API). При сворачивании чата — поллинг останавливается. При развороте — возобновляется с немедленным запросом.
