...
```
satcontact/
├── index.html           # SPA, шапка + карточки + mapView + arView + newsView + оверлей in-app браузера
├── style.css            # Telegram Dark, mobile-first, стили карты, AR, новостей и оверлея in-app
├── utils.js             # Общие утилиты (SatContactResolveUrl — резолв base URL)
├── app.js               # Парсинг XML, фильтры, рендер, SPA-маршрутизация, регистрация SW, авто-перезагрузка, детекция in-app
...
├── manifest.json        # PWA-манифест (standalone, portrait, theme/background #1c242d)
├── sw.js                # Service Worker: precache + runtime cache (tle.txt), стратегии fetch
├── icons/
│   ├── icon-180.png     # Apple Touch Icon 180×180
│   ├── icon-192.png     # Android manifest icon 192×192
│   ├── icon-512.png     # Android manifest + splash 512×512
│   └── icon-512-maskable.png  # Maskable-иконка 512×512 (safe zone 80%)
...
│   ├── board.html       # HTML-фрагмент доски объявлений (Модуль 4, редактируется админом через board-editor.html или GitHub)
│   └── board-media/     # Медиафайлы доски объявлений (копируются редактором или загружаются через GitHub; имена — латиница)
├── scripts/
...
├── board-editor.html    # Локальный WYSIWYG-редактор доски объявлений (File System Access API, Chrome/Edge). Исключён из Git
├── .gitignore           # Исключает board-editor.html, .cursor/, node_modules/ из репозитория
├── PROJECT_CONTEXT.md
...
- **`calibrating`**: камера и сенсоры запущены, рендеринг не идёт. Нижняя панель `#arCalibPanel` с двумя строками кнопок небесных тел (строка 1: Солнце/Луна/Полярная; строка 2: Венера/Юпитер/Сириус) и кнопкой «Зафиксировать». Кнопка «Назад» в шапке доступна. Доступность всех 6 тел определяется по elevation > 5° (`updateCelestialBodiesAvailability`, обновляется каждые 10 с). Кнопка «Зафиксировать» неактивна пока `areSensorsReady()` не вернёт true (кеш координат + сенсоры + камера) и не выбрано небесное тело. При пустом кеше — «Нет координат», кнопка «↻ Сеть» работает прямо в AR. «Зафиксировать» — калибровка по выбранному телу, `calibrationDelta` рассчитан.
...
- **data/board.html** — HTML-фрагмент доски объявлений, редактируемый админом через локальный визуальный редактор `board-editor.html` (основной способ) или через web-интерфейс GitHub.com (резервный способ).
- **data/board-media/** — медиафайлы доски (копируются автоматически редактором `board-editor.html` при добавлении медиа, или загружаются вручную через GitHub). Имена файлов — строго латиница (Jekyll/GitHub Pages игнорирует файлы, начинающиеся с `_`).
- **board-editor.html** — локальный однофайловый WYSIWYG-редактор доски объявлений (File System Access API, Chrome/Edge). Исключён из Git через `.gitignore`, хранится только на ПК админа.
...
**Точка входа:** кнопка `#newsBtn` («НОВОСТИ») в шапке Модуля 1 (`.header__top`). Стили: приглушённый полупрозрачный фон (`.btn--news`).

**Индикатор «НОВОСТИ» после релиза (build badge):**

После каждого деплоя с новым `CACHE_VERSION` кнопка «НОВОСТИ» подсвечивается анимацией `newsShimmer` (плавный 4-секундный цикл: синий → бирюзово-зелёный → тёплый золотистый). Подсветка снимается одним кликом по кнопке и не возвращается до следующего бампа `CACHE_VERSION`. "Тихих" релизов без подсветки не бывает.

Механизм (два слоя):

| Слой | Что | Где |
|---|---|---|
| A. Маркер релиза | Константа `CACHE_VERSION` | `app.js` (строка 9, дубль значения из `sw.js`) |
| B. Состояние пользователя | Последний `CACHE_VERSION`, для которого пользователь снял подсветку | `localStorage`, ключ `satcontact_news_dismissed` |

Логика:
- **Старт приложения (`init()`):** `isNewsDismissed()` сравнивает `localStorage['satcontact_news_dismissed']` с `CACHE_VERSION`. Если не совпадают или ключ отсутствует → класс `btn--news-updated` добавляется на `#newsBtn`.
- **Клик по кнопке:** `dismissNews()` записывает текущий `CACHE_VERSION` в localStorage, класс `btn--news-updated` снимается, затем открывается newsView.
- **Новый деплой:** `CACHE_VERSION` в коде изменился → не совпадает с сохранённым → подсветка снова активна.

Граничные случаи:
- `localStorage` недоступен (приватный режим, WebView) → `try/catch`, `isNewsDismissed()` возвращает `false` → подсветка горит всегда (безопасное поведение).
- Очистка данных сайта → localStorage сброшен → подсветка загорится повторно (приемлемо).
- `navigator.storage.persist()` вызывается в `init()` для защиты от автоматической эвикции хранилища браузером при нехватке места.

CSS (style.css): `.btn--news.btn--news-updated` — двойной класс для повышения специфичности. Анимация `@keyframes newsShimmer` — 4s ease-in-out infinite, три фазы с переходом `background-color` и `border-color`. Opacity каждой фазы подобрана для одинаковой воспринимаемой яркости на тёмном фоне.
...
### 7.6 Обновление доски (precache-only)

`data/board.html`, `data/Frequencies.xml` и `data/board-media/*` обновляются исключительно через деплой (push в GitHub Pages) + бамп `CACHE_VERSION` в `sw.js` **и** `app.js` (значения идентичны). Runtime-детекция обновлений, поллинг и уведомления (`handleBoardFetch`, `BOARD_UPDATED`, `triggerBoardCheck`) полностью удалены.

Бамп `CACHE_VERSION` выполняет две функции одновременно: (1) SW получает новый precache с обновлёнными файлами, (2) индикатор «НОВОСТИ» активируется на кнопке `#newsBtn` (см. 7.4).

**Синхронизация `PRECACHE_URLS` с `data/board-media/`:** при каждом бампе `CACHE_VERSION` необходимо проверить, какие файлы реально лежат в `data/board-media/`, и синхронизировать список `PRECACHE_URLS` в `sw.js`. Если в списке останется файл, которого нет на диске, `cache.addAll()` получит 404 и установка SW сломается для всех пользователей. Правило описано в `.cursor/rules/pwa-cache.mdc`.

**Рабочий процесс обновления доски:**
1. Админ редактирует `data/board.html` через `board-editor.html` (WYSIWYG, File System Access API, Chrome/Edge) или напрямую через GitHub.
2. Медиафайлы автоматически копируются редактором в `data/board-media/` (латинские имена).
3. Агенту даётся команда «Бампни CACHE_VERSION» — он синхронизирует `PRECACHE_URLS`, бампит версию в обоих файлах, обновляет правило.
4. `git add . && git commit && git push` — GitHub Pages публикует, пользователи получают обновление.

### 7.7 Кеширование

**Service Worker Cache Storage (два кэша):**

| Кэш | Имя | Удаляется при обновлении SW? | Содержимое |
|------|-----|-----|------|
| Precache | `satcontact-vN` (версионируется) | Да — старая версия удаляется в `activate` | App shell: HTML, CSS, JS, XML, JSON, board.html, board-media, иконки, манифест |
| Runtime | `satcontact-runtime` (постоянный) | Нет — живёт между версиями SW | `data/tle.txt` |

| Ресурс | Стратегия SW | Кэш |
|--------|-------------|------|
| `index.html`, `*.js`, `*.css`, `manifest.json`, иконки | Cache First (precache) | `satcontact-vN` |
| `data/countries-50m.json`, `data/Frequencies.xml`, `data/board.html`, `data/board-media/*` | Cache First (precache) | `satcontact-vN` |
| `data/tle.txt` | Network First | `satcontact-runtime` |
| `*.backblazeb2.com/*`, `ipwhois.app/*`, `geolocation-db.com/*` | Пропуск (cross-origin, напрямую в сеть) | — |

**Клиентское кеширование (без изменений):**
...
Runtime-детекция обновлений доски удалена — `board.html` обновляется через precache (см. 7.6).

---

## 8. PWA — Service Worker, манифест, офлайн (реализовано)

### 8.1 Обзор

Приложение работает как Progressive Web App: офлайн-кеширование, установка на домашний экран, автоматическое обновление при деплое. Обновления редкие (~раз в 2 месяца), версия `CACHE_VERSION` меняется вручную.

**Критически важно:** `CACHE_VERSION` определена в **двух файлах** — `sw.js` (строка 1) и `app.js` (строка 9, внутри IIFE). Значения должны быть **идентичны**. `sw.js` использует константу для имени кеша, `app.js` — для индикатора «НОВОСТИ» (сравнение с localStorage). Правило автоматического бампа описано в `.cursor/rules/pwa-cache.mdc`.

### 8.2 Service Worker (sw.js)

**Двойной кэш:** `satcontact-vN` (versioned precache, app shell включая `board.html` и `board-media/*`) + `satcontact-runtime` (persistent, только `data/tle.txt`). При обновлении SW старый precache удаляется в `activate`, runtime переживает обновления. Количество файлов в precache динамическое — зависит от содержимого `data/board-media/`.

**Событие install:** `cache.addAll()` с `{ cache: 'reload' }` в каждом Request — обходит HTTP-кеш браузера и CDN GitHub Pages.

**Событие activate:** удаляет все кэши кроме текущего precache и runtime. `self.clients.claim()` — немедленный контроль открытых вкладок.

**Событие fetch — маршрутизация:**
- Cross-origin (`url.origin !== self.location.origin`) → пропуск, запрос идёт напрямую в сеть.
- `data/tle.txt` → `networkFirst()` → `satcontact-runtime`.
- Всё остальное → `cacheFirst()` → поиск во всех кэшах.

**networkFirst:** `fetch(request, { cache: 'no-store' })` — полностью обходит HTTP-кеш браузера и CDN GitHub Pages (max-age ~10 мин). Используется только для `tle.txt`.

**skipWaiting + clients.claim:** новый SW активируется немедленно без запроса к пользователю.

### 8.3 Регистрация SW и авто-перезагрузка (app.js)

Блок в начале IIFE `app.js` (до объявления переменных):
1. `CACHE_VERSION` — дубль значения из `sw.js`, используется для индикатора «НОВОСТИ» (см. 7.4).
2. При загрузке запоминается `navigator.serviceWorker.controller`.
3. Слушатель `controllerchange` — если SW обновился (`skipWaiting` + `clients.claim`), флаг сбрасывается в `null`. При `visibilitychange → visible` с `null`-флагом → `location.reload()`.
4. `navigator.serviceWorker.register('sw.js')`.

**Persistent Storage:** в `init()` вызывается `navigator.storage.persist()` — запрос к браузеру не удалять хранилище origin автоматически при нехватке места. Защищает localStorage (dismiss-флаг индикатора, группу, позывной) и Cache Storage от эвикции. На обновления SW и кешей не влияет — защита только от автоматической очистки браузером, не от ручной.

iOS-специфика: при возврате к свёрнутому PWA система часто убивает процесс — пользователь автоматически получает свежую версию. `visibilitychange` страхует случай, когда iOS оставил процесс живым.

### 8.4 Манифест (manifest.json)

`display: standalone`, `orientation: portrait`, `start_url: /satcontact/`, `scope: /satcontact/`, `theme_color` и `background_color`: `#1c242d` (совпадает с `--bg-app`). Иконки: 192×192, 512×512, 512×512 maskable.

Android генерирует splash автоматически из манифеста. iOS splash (`apple-touch-startup-image`) отложен на следующий этап.

### 8.5 Мета-теги PWA (index.html)
...
### 8.6 Оверлей для in-app браузеров

При открытии в WebView мессенджера (Telegram, Facebook, Instagram, WhatsApp, Line, Twitter/X, Snapchat) — блокирующий оверлей с инструкцией открыть в стандартном браузере.

**Детекция (`isInAppBrowser()` в app.js):** проверка User-Agent на маркеры мессенджеров (`FBAN`, `FBAV`, `Telegram`, `WhatsApp` и др.), Android WebView (`;wv)`), iOS WKWebView (наличие `AppleWebKit` без `Safari/`).

**HTML:** `<div id="inappOverlay" hidden>` — первый дочерний элемент `<div class="app">`. CSS: `position: fixed; inset: 0; z-index: 9999; backdrop-filter: blur(6px)`.

**Логика:** в начале `init()` — если `isInAppBrowser()`, оверлей показывается, `init()` возвращается (`return`). Приложение не инициализируется, fetch-запросы не выполняются.

### 8.7 Поток обновления

1. Разработчик обновляет файлы (board.html, Frequencies.xml, код и т.д.).
2. Агент или разработчик бампит `CACHE_VERSION` в **обоих** файлах (`sw.js` + `app.js`, одинаковое значение), синхронизирует `PRECACHE_URLS` с реальными файлами в `data/board-media/`.
3. `git push` → GitHub Pages публикует.
4. Браузер проверяет `sw.js` → файл изменился → новый SW: install (precache с `cache: 'reload'`) → `skipWaiting` → activate (удаление старого кэша) → `clients.claim`.
5. Событие `controllerchange` в клиенте → флаг сбрасывается.
6. При возврате в приложение (`visibilitychange → visible`) → `location.reload()` → страница из нового кэша.
7. Новый `app.js` загружен → `CACHE_VERSION` не совпадает с `localStorage['satcontact_news_dismissed']` → кнопка «НОВОСТИ» подсвечивается анимацией `newsShimmer`.
8. Пользователь кликает «НОВОСТИ» → подсветка снимается, dismiss записывается в localStorage → до следующего бампа подсветки нет.

### 8.8 Precache-список

Актуальный список `PRECACHE_URLS` ведётся в `sw.js` и дублируется в `.cursor/rules/pwa-cache.mdc`. Количество файлов динамическое — зависит от содержимого `data/board-media/`.

Базовый набор (без board-media): `./` (index.html), `style.css`, `utils.js`, `app.js`, `tle.js`, `tle-worker.js`, `gps-service.js`, `map.js`, `map-render.js`, `ar.js`, `ar-render.js`, `news.js`, `lib/satellite.min.js`, `lib/d3.min.js`, `lib/topojson.min.js`, `lib/aws4fetch.min.js`, `data/Frequencies.xml`, `data/countries-50m.json`, `data/board.html`, `manifest.json`, `icons/icon-192.png`, `icons/icon-512.png`, `icons/icon-180.png` + все файлы из `data/board-media/` (кроме `.gitkeep`).

НЕ в precache: `README.md`, `PROJECT_CONTEXT.md`, `MODUL4_PROJECT_CONTEXT.md`, `CHAT_OPTIMIZATION_SPEC.md`, `lib/README.md`, `icons/icon-512-maskable.png` (браузер скачивает сам при установке), документация, CI-скрипты.

## 9. ЧТО НЕ РЕАЛИЗОВАНО (следующие этапы)

- **iOS splash images** (`apple-touch-startup-image`) — фаза 2.
- **IndexedDB** для офлайн-хранения сообщений чата.
- **Background Sync** для отправки сообщений при восстановлении сети.
- **Periodic Background Sync** для периодической проверки обновлений.

---

## 10. ИСТОРИЯ СЕССИЙ (для контекста при продолжении)
...
2. **Новые переменные:** `calibState` (строка), `selectedCalibBody` (`'sun'|'moon'|'polaris'|'venus'|'jupiter'|'sirius'|null`), `celestialAvailTimerId`.
...
5. **Новые функции (13 штук):**
   ...
   - `updateCelestialBodiesAvailability()` — вычисление elevation 6 тел (Солнце/Луна/Полярная/Венера/Юпитер/Сириус), enabled/disabled кнопок
   - `syncBodySelection()` — визуальное состояние 'selected' на кнопках 6 тел
...
15. **Новые DOM-ссылки в `cacheDom()`:** `elCalibWaitGps`, `elCalibPhase1`, `elCalibPhase2`, `elPhase1Progress`, `elPhase1Text`, `elCalibSun`, `elCalibMoon`, `elCalibPolaris`, `elCalibVenus`, `elCalibJupiter`, `elCalibSirius`, `elPhase2CompassToggle`, `elCalibFixBtn`, `elPhase2NoBodies`, `elRecalibBtn`. Удалены: `elCalibSource`, `elCalibBtn`, `elCompassToggle`, `elPhase2Instruction`.
...
- UI: три полупрозрачных оверлея поверх видео камеры для каждой фазы, SVG-кольцо прогресса, 6 кнопок небесных тел (2 строки × 3) с авто-доступностью.
...
**Не изменено:** `computeOrientationMatrix`, WMM (`getMagneticDeclination`, `WMM_COEFFS`), астрономия (`getSunAzEl`, `getMoonAzEl`, `getPolarisAzEl`), drift timer (`getDriftError`, `updateDriftIndicator`), audio, overview/focus, rendering pipeline, Worker trajectories, camera, GPS.
...
- `data/board.html` — HTML-фрагмент доски объявлений (редактируется админом через `board-editor.html`).
- `data/board-media/.gitkeep` — папка для медиафайлов доски (файлы копируются редактором).
...
- В `app.js`: `#headerTitle` заменён на `#newsBtn`, убран keydown-обработчик (кнопка нативно реагирует на Enter/Space).
...
- Кнопка «Обновить» (`#newsRefresh`) обновляет только чат (board.html обновляется через precache).

**4. Детекция обновлений доски** — удалена. `board.html` переведён на precache-only стратегию (см. раздел 7.6).
...
### Сессия: Локальный визуальный редактор доски объявлений (март 2026)

**Цель сессии:**
- Создать удобный локальный WYSIWYG-редактор для `data/board.html`, чтобы админ мог визуально редактировать доску объявлений и добавлять медиа без ручного написания HTML.
- Редактор не должен попадать в репозиторий (остаётся только на ПК админа).

**Созданные файлы:**
- `board-editor.html` — однофайловый локальный WYSIWYG-редактор доски объявлений. Использует File System Access API (Chrome/Edge). Возможности: открытие папки проекта → автоматическое нахождение `data/board.html`, визуальное редактирование (contentEditable), панель форматирования (жирный, курсив, ссылки, разделители), добавление изображений/видео/файлов (кнопки + drag & drop) с автокопированием в `data/board-media/` и вставкой корректных HTML-тегов, просмотр и редактирование HTML-исходника, панель превью, список файлов в `board-media/`, сохранение в `data/board.html`. Файл исключён из Git.
- `.gitignore` — исключает `board-editor.html` из репозитория.

**Рабочий процесс админа (обновлённый):**
1. Открыть `board-editor.html` в Chrome/Edge
2. «Открыть папку проекта» → выбрать корневую папку SatContact
3. Редактировать текст визуально, добавлять медиа кнопками или drag & drop
4. Медиафайлы автоматически копируются в `data/board-media/` с латинскими именами
5. «Сохранить» → `git add . && git commit && git push`
6. Обновление на сайте ~1-2 мин

**Обнаруженная проблема:** GitHub Pages по умолчанию использует Jekyll, который игнорирует файлы с именами, начинающимися с `_` (нижнее подчёркивание). Функция санитизации имён в редакторе заменяет нелатинские символы на `_`, что приводило к невидимости файлов на GitHub Pages (404). Решение: имена медиафайлов должны содержать латинские символы; при кириллических именах файл может начаться с `_` и не будет опубликован Jekyll. Альтернативное решение — добавить файл `.nojekyll` в корень репозитория для отключения Jekyll.

**Обновлена документация:** PROJECT_CONTEXT.md (дерево файлов, секция 7.1, история сессий), MODUL4_PROJECT_CONTEXT.md (рабочий процесс админа, таблица файлов, инструкции по подготовке).

**Файлы изменены:** board-editor.html (новый), .gitignore (новый), PROJECT_CONTEXT.md, MODUL4_PROJECT_CONTEXT.md.

**Файлы НЕ изменены:** index.html, style.css, app.js, news.js, map.js, map-render.js, ar.js, ar-render.js, tle.js, tle-worker.js, gps-service.js, utils.js, lib/aws4fetch.min.js, data/board.html.

### Сессия: Расширение калибровки — Венера, Юпитер, Сириус (март 2026)

**Цель сессии:**
- Расширить набор калибровочных небесных тел с 3 (Солнце/Луна/Полярная) до 6 (+ Венера/Юпитер/Сириус) для повышения доступности калибровки в любое время суток.
- Освободить место на панели: удалить текстовую подсказку «Направьте камеру…».
- Использовать для новых тел дизайн прицела, аналогичный Полярной звезде (белый, dashed, пульсация, внутреннее кольцо, перекрестие).

**Новые астрономические функции (ar.js):**
- `getSiriusAzEl(observer, date)` — фиксированные RA/Dec (6h 45m 09s, −16° 42' 58"), через существующую `raDecToAzEl`. Аналогично `getPolarisAzEl`, только другие координаты. Сириус — не околополярная звезда, восходит и заходит, но математика идентична.
- `getVenusAzEl(observer, date)` — аналитическая формула по Жану Мёэсу (Jean Meeus). Вычисляет гелиоцентрические долготы и радиусы Земли и Венеры через орбитальные элементы + уравнение центра, затем геоцентрические прямоугольные координаты, перевод из эклиптики в экватор (наклон эклиптики), RA/Dec → `raDecToAzEl`. Венера движется в плоскости эклиптики (наклонение ~3.4° не учитывается для low-precision). Точность ~0.1°.
- `getJupiterAzEl(observer, date)` — аналогичная формула. Юпитер имеет заметную эклиптическую широту (~1.3°), которая учтена (`jupLatDeg`). Уравнение центра с 3 членами. Точность ~0.1°.

**Изменения в UI (index.html):**
- Добавлена вторая строка `div.ar-view__phase2-bodies` с 3 кнопками: `#arCalibVenus` (☆ Венера), `#arCalibJupiter` (♃ Юпитер), `#arCalibSirius` (✧ Сириус).
- Полностью удалён элемент `#arPhase2Instruction` (`<p class="ar-view__calib-panel-sub">`).

**Изменения в стилях (style.css):**
- Удалён блок `.ar-view__calib-panel-sub`.
- Добавлены стили прицелов `--venus`, `--jupiter`, `--sirius` — группированные селекторы, полностью повторяющие дизайн `--polaris`: 100×100 dashed-круг, radial-gradient центр, голубой drop-shadow, анимация `calibCrosshairPulse`, внутреннее кольцо 40×40, горизонтальное/вертикальное перекрестие.

**Изменения в логике (ar.js):**
- Новые DOM-переменные: `elCalibVenus`, `elCalibJupiter`, `elCalibSirius`.
- Удалена переменная `elPhase2Instruction`.
- `selectedCalibBody` расширен: `'sun'|'moon'|'polaris'|'venus'|'jupiter'|'sirius'|null`.
- `cacheDom()` — 3 новые ссылки, удалена ссылка на `arPhase2Instruction`.
- `bindUi()` / `unbindUi()` — подписка/отписка click-обработчиков для 3 новых кнопок.
- `updateCelestialBodiesAvailability()` — вычисляет elevation всех 6 тел, управляет disabled/enabled 6 кнопок, сбрасывает выбор если тело ушло за горизонт, проверка `noneAvailable` по всем 6.
- `syncBodySelection()` — итерация по массиву из 6 кнопок.
- `onSelectCelestialBody()` — удалён вызов `updatePhase2Instruction()`.
- `onFixCalibration()` — вместо цепочки if/else используется lookup-объект `bodyFn` с маппингом 6 тел на соответствующие функции расчёта.
- Полностью удалена функция `updatePhase2Instruction()`.

**Файлы изменены:** ar.js, index.html, style.css, PROJECT_CONTEXT.md.

**Файлы НЕ изменены:** app.js, news.js, map.js, map-render.js, ar-render.js, tle.js, tle-worker.js, gps-service.js, utils.js, lib/aws4fetch.min.js, data/board.html, board-editor.html.

### Сессия: Миграция на PWA — Service Worker, манифест, оверлей in-app (март 2026)

**Цель сессии:**
- Перевести приложение в режим Progressive Web App: офлайн-работа, установка на домашний экран, автоматическое обновление при деплое.
- Перенести детекцию обновлений доски из клиентского `checkBoardForUpdates()` в Service Worker с архитектурой двух кэшей.
- Добавить блокирующий оверлей для in-app браузеров мессенджеров.

**1. Новые файлы**
- `manifest.json` — PWA-манифест (`standalone`, `portrait`, `theme_color`/`background_color`: `#1c242d`, scope `/satcontact/`).
- `sw.js` — Service Worker с двойным кэшем: `satcontact-v7` (versioned precache, 24 файла включая `board.html`) + `satcontact-runtime` (persistent, Network First только для `tle.txt`). Стратегии: Cache First для app shell и данных, Network First для `tle.txt`, пропуск cross-origin. `skipWaiting()` + `clients.claim()` для немедленной активации.
- `icons/icon-180.png` (180×180), `icon-192.png` (192×192), `icon-512.png` (512×512), `icon-512-maskable.png` (512×512 maskable) — иконки приложения, фон `#1c242d`, акцент `#5288c1`, стилизованная спутниковая антенна.

**2. index.html**
- В `<head>`: `<link rel="manifest">`, `<meta name="apple-mobile-web-app-capable">`, `<meta name="apple-mobile-web-app-status-bar-style">`, `<link rel="apple-touch-icon">`.
- Первый дочерний элемент `<div class="app">`: `<div id="inappOverlay" hidden>` — оверлей in-app браузера с инструкцией открыть в Chrome/Safari.

**3. style.css**
- Добавлены стили `.inapp-overlay` (fixed, inset: 0, z-index: 9999, backdrop-filter: blur), `.inapp-overlay__box`, `.inapp-overlay__icon`, `.inapp-overlay__text`, `.inapp-overlay__hint`.

**4. app.js**
- В начало IIFE (после `'use strict'`): блок регистрации SW — `controllerchange` (сброс флага + авто-перезагрузка при возврате), `register('sw.js')`.
- Новая функция `isInAppBrowser()` — детекция WebView мессенджеров по User-Agent.
- В начало `init()`: проверка in-app → показ оверлея + `return` (блокировка инициализации).
- Замена `checkBoardForUpdates()` на `fetch(window.SatContactResolveUrl('data/board.html')).catch(function() {})` — триггер SW.

**5. news.js**
- Удалён TODO-комментарий (строка 74): `// TODO [PWA]: migrate checkBoardForUpdates to Service Worker for background detection`.
- Удалён экспорт `window.checkBoardForUpdates`.
- Удалён TODO-комментарий (строка 260): `// TODO [PWA]: migrate to Service Worker for background board update detection`.
- Удалена функция `checkBoardForUpdates()` целиком (строки 261-273).
- `loadBoard()` упрощён — без localStorage-кеша и шиммера, только fetch → render → error fallback.

**6. .gitignore**
- Добавлены: `.cursor/`, `node_modules/`, `package.json`, `package-lock.json`.

**Решённые проблемы (из PROJECT_CONTEXT.md раздел 7.6):**
- Однократная проверка при старте → SW детектирует при каждом fetch `board.html` (старт, открытие доски, авто-перезагрузка).
- Баг пустого кэша → SW сравнивает только при наличии кэшированной версии, первый визит сохраняет baseline без уведомления.
- CDN GitHub Pages → `fetch(request, { cache: 'no-store' })` полностью обходит HTTP-кэш и CDN.

**Файлы созданы:** manifest.json, sw.js, icons/icon-180.png, icons/icon-192.png, icons/icon-512.png, icons/icon-512-maskable.png.

**Файлы изменены:** index.html, style.css, app.js, news.js, .gitignore, PROJECT_CONTEXT.md.

**Файлы НЕ изменены:** utils.js, tle.js, tle-worker.js, gps-service.js, map.js, map-render.js, ar.js, ar-render.js, lib/*, data/*, scripts/*, .github/workflows/*.

### Сессия: Исправление механизма обновлений PWA (март 2026)

**Цель сессии:**
- Исправить три бага в механизме кеширования Service Worker, из-за которых пользователи PWA не получали обновления данных.
- Создать Cursor Rule для автоматического напоминания о бампе `CACHE_VERSION`.

**1. sw.js — маршрут Frequencies.xml через networkFirst**
- `data/Frequencies.xml` был заперт в стратегии `cacheFirst` (через `PRECACHE_URLS`). XML редактируется часто и независимо от кода приложения, но пользователь не получал обновлённый файл без бампа SW.
- Добавлен отдельный маршрут в `fetch`-обработчике (между `board-media` и финальным `cacheFirst`): `url.pathname.endsWith('/data/Frequencies.xml')` → `networkFirst(event.request, RUNTIME_CACHE)`.
- Файл остаётся в `PRECACHE_URLS` для офлайн-baseline при первой установке, но при наличии сети отдаётся свежая версия через `satcontact-runtime`.

**2. sw.js — исправление networkFirst (HTTP-кеш)**
- Функция `networkFirst` использовала `fetch(request)` без `{ cache: 'no-store' }`. По умолчанию `cache: 'default'` — браузер мог вернуть ответ из HTTP-кеша (max-age ~10 мин GitHub Pages), не обращаясь к сети. Название «networkFirst», но фактическое поведение — HTTP-cache-first.
- Исправлено: `fetch(request, { cache: 'no-store' })`. Затрагивает `tle.txt`, `board-media/*`, `Frequencies.xml`.

**3. sw.js — бамп CACHE_VERSION**
- `CACHE_VERSION` изменён с `satcontact-v1` на `satcontact-v2`. Без этого браузер не обнаруживает изменение sw.js и не устанавливает новый Service Worker.

**4. Cursor Rule (.cursor/rules/pwa-cache.mdc)**
- Создан файл правила с `alwaysApply: true`. Содержит список всех 23 precache-файлов и инструкции: при изменении любого файла из списка — бампить `CACHE_VERSION` в sw.js; при архитектурных изменениях — обновлять `PROJECT_CONTEXT.md`.

**Ранее применённые исправления (в текущем коде, НЕ менялись в этой сессии):**
- `handleBoardFetch` и runtime-детекция обновлений доски полностью удалены — `board.html` переведён на precache-only.

**Файлы изменены:** sw.js, PROJECT_CONTEXT.md.

**Файлы созданы:** .cursor/rules/pwa-cache.mdc.

**Файлы НЕ изменены:** app.js, index.html, style.css, news.js, utils.js, tle.js, tle-worker.js, gps-service.js, map.js, map-render.js, ar.js, ar-render.js, lib/*, data/*, scripts/*, .github/workflows/*.

### Сессия: Индикатор «НОВОСТИ» после релиза + усиление правила CACHE_VERSION (март 2026)

**Цель сессии:**
- Реализовать build badge: после каждого деплоя кнопка «НОВОСТИ» подсвечивается анимацией, пока пользователь не кликнет по ней.
- Обеспечить автоматическую синхронизацию `PRECACHE_URLS` с реальными файлами `data/board-media/` при бампе версии.
- Усилить Cursor Rule для надёжности будущих деплоев.

**1. Индикатор «НОВОСТИ» (app.js, style.css)**
- Константа `CACHE_VERSION` добавлена в `app.js` (строка 9, внутри IIFE) — дубль значения из `sw.js`. Используется для сравнения с `localStorage['satcontact_news_dismissed']`.
- Функции `isNewsDismissed()` и `dismissNews()` — обёрнуты в `try/catch` (приватный режим Safari, WebView). При ошибке чтения — подсветка горит (безопасное поведение).
- В `init()`: если `!isNewsDismissed()` → класс `btn--news-updated` на `#newsBtn`. Клик → `dismissNews()` + снятие класса + `openNewsView()`.
- CSS: `.btn--news.btn--news-updated` — анимация `newsShimmer` 4s ease-in-out infinite, три фазы: синий `rgba(82,136,193)` → бирюзово-зелёный `rgba(72,172,126)` → тёплый золотистый `rgba(196,170,56)`. Opacity подобрана для одинаковой воспринимаемой яркости.

**2. Persistent Storage (app.js)**
- `navigator.storage.persist()` вызывается в `init()` — защита от автоматической эвикции хранилища браузером при нехватке места. Не влияет на обновления SW и кешей.

**3. Cursor Rule (.cursor/rules/pwa-cache.mdc) — усиление**
- Правило 1 обновлено: `CACHE_VERSION` бампится в **обоих** файлах (`sw.js` строка 1, `app.js` строка 9), значения идентичны.
- Добавлено правило 2: при бампе `CACHE_VERSION` агент обязан проверить реальные файлы в `data/board-media/`, синхронизировать `PRECACHE_URLS` (добавить новые, удалить отсутствующие), обновить список в файле правила. Без этого `cache.addAll()` может получить 404 и SW не установится.

**4. PROJECT_CONTEXT.md — обновление документации**
- Секция 7.4: добавлено описание индикатора (механика, два слоя, граничные случаи, CSS).
- Секция 7.6: добавлен рабочий процесс обновления доски и синхронизация PRECACHE_URLS.
- Секция 7.7: заменены устаревшие ссылки `satcontact-v7` на `satcontact-vN`.
- Секция 8.1: добавлено предупреждение о двух файлах CACHE_VERSION.
- Секция 8.2: обновлено описание precache (динамическое количество файлов).
- Секция 8.3: добавлены CACHE_VERSION в app.js и persistent storage.
- Секция 8.7: поток обновления расширен шагами 7-8 (индикатор).
- Секция 8.8: precache-список описан как динамический.

**Файлы изменены:** app.js, style.css, sw.js, .cursor/rules/pwa-cache.mdc, PROJECT_CONTEXT.md.

**Файлы НЕ изменены:** index.html, news.js, utils.js, tle.js, tle-worker.js, gps-service.js, map.js, map-render.js, ar.js, ar-render.js, lib/*, data/*, scripts/*, .github/workflows/*.
