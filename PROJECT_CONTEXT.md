...
- **`phase1`** (детекция гироскопа, compass-free): `gyroCorrection` загружается из `localStorage` (кеш) через `loadGyroCorrection()`, `startGyroDetection()` запускает фазу детекции. Три пары аккумуляторов параллельно: pitch (compass-free, `detectBetaOs`/`detectBetaGyro`), roll (compass-free, `detectGammaOs`/`detectGammaGyro`), heading (fallback, `detectOsAccum`/`detectGyroAccum`). SVG-кольцо `#arPhase1Progress` показывает прогресс (max из трёх пар, цвет red→green). Двухшаговый UI: «Наклоните телефон вперёд-назад» → «Теперь медленно поверните влево-вправо» (при 5° pitch). Автопереход в `phase2` при `|detectBetaOs| ≥ 10° ИЛИ |detectGammaOs| ≥ 10° ИЛИ |detectOsAccum| ≥ 15°` через `finalizeGyroDetection()` (4-step cascade) → `transitionToPhase2()`. Таймаута нет. Потеря GPS → `waiting_gps`.
