...
|------|-----|-----|------|
| Precache | `satcontact-v1` (версионируется) | Да — старая версия удаляется в `activate` | App shell: HTML, CSS, JS, XML, JSON, иконки, манифест (23 файла) |
| Runtime | `satcontact-runtime` (постоянный) | Нет — живёт между версиями SW | `data/tle.txt`, `data/board.html`, `data/board-media/*` |

| Ресурс | Стратегия SW | Кэш |
|--------|-------------|------|
| `index.html`, `*.js`, `*.css`, `manifest.json`, иконки | Cache First (precache) | `satcontact-v1` |
| `data/Frequencies.xml`, `data/countries-50m.json` | Cache First (precache) | `satcontact-v1` |
...
**Двойной кэш:** `satcontact-v1` (versioned precache, 23 файла app shell) + `satcontact-runtime` (persistent, данные Network First). При обновлении SW старый precache удаляется в `activate`, runtime переживает обновления.
...
- `data/board.html` → `handleBoardFetch()` — Network First + детекция обновлений + `postMessage` (см. 7.6).
- `data/tle.txt`, `data/board-media/*` → `networkFirst()` → `satcontact-runtime`.
...
3. Слушатель `visibilitychange` — при возврате в приложение (`visible`) + флаг `null` → `location.reload()`. Страница загружается из нового кэша.
