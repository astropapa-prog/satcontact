...
Модуль 4 «Новости» — четвёртый экран SPA. Состоит из двух частей: доска объявлений администратора (верхняя, сворачиваемая секция) и чат-лента сообщений.

- **news.js** — вся логика модуля: доска, чат, авторизация по позывному, CRUD сообщений, поллинг, сжатие изображений.
...
- **app.js** — `openNewsView()` / `closeNewsView()`, кнопка входа — заголовок `SAT Навигатор` (`#headerTitle`).
...
Каждое сообщение — отдельный JSON-файл `{timestamp}-{author_hash}-msg.json`. Медиафайлы — отдельные бинарные объекты `{timestamp}-{author_hash}-{filename}.{ext}`. Автоочистка: Backblaze B2 Lifecycle Rules (3 дня → hide, 4 дня → delete).

### 7.3 Идентификация пользователя

Позывной вводится при первом входе в модуль. Хеш (SHA-256, первые 8 символов) вычисляется через SubtleCrypto и хранится в localStorage. Проверка авторства: `msg.author_hash === localUserHash`. Свои сообщения отображаются с другим фоном и кнопками редактирования/удаления.

### 7.4 SPA-переход

Идентичен модулям 2 и 3: `openNewsView()` скрывает header + main, показывает `#newsView`, вызывает `window.initNews()`. `closeNewsView()` — обратный переход, вызывает `window.cleanupNews()`. Точка входа — клик по заголовку `SAT Навигатор` в шапке Модуля 1.

### 7.5 Кеширование

| Что | Где | TTL |
|-----|-----|-----|
| Листинг файлов (ListObjectsV2) | sessionStorage | 90 сек |
| HTML доски (board.html) | localStorage | До следующего открытия модуля |
...
Каждые 90 секунд — фоновый ListObjectsV2. Новые сообщения добавляются в ленту, удалённые — убираются из DOM. При скрытой вкладке поллинг приостанавливается (Visibility API). При возвращении — немедленный запрос.
...
**Созданные файлы:**
- `news.js` — IIFE-модуль (~500 строк): конфигурация B2, состояние чата, жизненный цикл (`initNews`/`cleanupNews`), загрузка доски (`loadBoard`, localStorage-кеш), авторизация по позывному (SHA-256 через SubtleCrypto), листинг S3 (`fetchFileList`, sessionStorage-кеш 90 сек), рендер сообщений (`createMessageElement`, `relativeTime`, `formatFileSize`), отправка (`sendMessage`, `compressImage` — canvas resize 1280px, JPEG 0.75), редактирование (PUT overwrite), удаление (DELETE JSON + медиа + вложения), поллинг 90 сек с Visibility API, fullscreen-просмотр изображений.
